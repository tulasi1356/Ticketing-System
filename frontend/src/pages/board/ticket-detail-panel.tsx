...
import { uploadAttachmentBlob } from "../../api/attachmentApi"
...
import { useTicketComments } from "../../hooks/comments/useTicketComments"
// import { useAdminTicketExport } from "../../hooks/useAdminTicketExport"
import { useUpdateTicket } from "../../hooks/tickets/useUpdateTicket"
import { cn } from "../../lib/utils"
import type { ProjectUser, Sprint, Ticket } from "../../types/ticketing"
import { useSprintsForProject } from "../../hooks/sprints/useSprintsForProject"
...
export function TicketDetailPanel({
  ...
  const { data: comments, isLoading: loadingComments, isError: commentsError } = useTicketComments(
    ticket.id
  )
  const {
    data: sprintsForTicketProject,
    isLoading: loadingSprintsForTicketProject,
    isError: sprintsErrorForTicketProject,
  } = useSprintsForProject(ticket.project_id ?? 0)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Could not save ticket")
    }
  }
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm lg:sticky ">
          ...
          <div className="divide-y divide-gray-100">
            <SidebarRow label="Priority">
              {editing ? (
                <Select
                  items={TICKET_PRIORITY_SELECT_OPTIONS}
                  value={draftPriority}
                  onChange={(v) => setDraftPriority(v as TicketPriority)}
                  className="w-full"
                />
              ) : (
            ...
            <SidebarRow label="Status">
              {editing ? (
                <Select
                  items={TICKET_STATUS_SELECT_OPTIONS}
                  value={draftStatus}
                  onChange={(v) => setDraftStatus(v as TicketStatus)}
                  className="w-full"
                />
              ) : (
            ...
            <SidebarRow label="Issue type">
              {editing ? (
                <Select
                  items={TICKET_ISSUE_TYPE_SELECT_OPTIONS}
                  value={draftIssueType}
                  onChange={(v) => setDraftIssueType(v as TicketIssueType)}
                  className="w-full"
                />
              ) : (
                issueTypeDisplayLabel(ticket.issue_type)
              )}
            </SidebarRow>
            ...
            {sprintName ? <SidebarRow label="Sprint">
              {editing ? (
                loadingSprintsForTicketProject ? (
                  ...
                  <Select
                    items={sprintsForTicketProject.map((s: Sprint) => ({
                      label: s.name,
                      value: s.id,
                    }))}
